...
and only then runs the release's local `install.sh`. It does not stream an
unverified archive into a shell or tar process. Pinned versions never fall
back silently to latest.
